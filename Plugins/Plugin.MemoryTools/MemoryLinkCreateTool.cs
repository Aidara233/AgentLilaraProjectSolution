...
public class MemoryLinkCreateTool : ITool
{
    ...
    [
        ...
        new("to_id", "目标记忆 ID", 1),
        new("strength", "（可选）关联强度 0.0-1.0，默认 1.0", 2, false),
    ...
    public async Task<ToolResult> ExecuteAsync(List<string> inputs, CancellationToken ct)
    {
        ...
        var strength = inputs.Count > 2 && float.TryParse(inputs[2], out var s) ? s : 1.0f;
        ...
        await memory.LinkAsync(fromId, toId, strength, linkType);
        return Ok($"已创建关联: {fromId} ↔ {toId} (strength={strength:F2}, type={linkType})");
    }
