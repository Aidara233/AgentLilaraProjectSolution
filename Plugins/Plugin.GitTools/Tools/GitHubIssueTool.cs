...
[ToolMeta(Group = "github", ContinueLoop = true, CapabilitySummary = "管理 GitHub Issues")]
public class GitHubIssueTool : GitToolBase
{
    private readonly GitHubClient _gh;
    private readonly RepoRegistry _repos;

    public GitHubIssueTool(string workspaceDir, GitRunner runner, GitHubClient gh, RepoRegistry repos)
    ...
    public override string Name => "github_issue";
    public override string Description => "管理 GitHub Issue。action: list(列出Issue) / create(创建Issue) / view(查看Issue) / close(关闭Issue)。";
    public override IReadOnlyList<ToolParameter> Parameters =>
    [
        new("repo", "仓库路径（相对于 Workspace 目录）", 0),
        new("action", "操作类型: list / create / view / close", 1),
        new("number", "（可选）Issue 编号（view/close 需要）", 2, isRequired: false),
        new("title", "（可选）Issue 标题（create 需要）", 3, isRequired: false),
        new("body", "（可选）Issue 描述（create 需要）", 4, isRequired: false),
    ];
    ...
    public override async Task<ToolResult> ExecuteAsync(List<string> resolvedInputs, CancellationToken ct)
    {
        ...
        return action switch
        {
            ...
            "close" => await DoClose(owner, repoName, resolvedInputs, ct),
            _ => Fail($"未知 action: {action}，可选: list / create / view / close")
        };
    }
    ...
    private async Task<ToolResult> DoList(string owner, string repo, List<string> inputs, CancellationToken ct)
    {
        ...
        var json = await _gh.ListIssues(owner, repo, state, ct);
        if (json.StartsWith("GitHub API")) return Fail(json);

        var sb = new StringBuilder();
        sb.AppendLine($"[{owner}/{repo}] Issue 列表 (state={state}):");
        try
        {
    ...
    private async Task<ToolResult> DoCreate(string owner, string repo, List<string> inputs, CancellationToken ct)
    {
        ...
        var json = await _gh.CreateIssue(owner, repo, title, body, ct);
        if (json.StartsWith("GitHub API")) return Fail(json);
        return Ok($"[{owner}/{repo}] Issue 创建成功:\n{FormatIssueResponse(json)}");
    }
    ...
    private async Task<ToolResult> DoView(string owner, string repo, List<string> inputs, CancellationToken ct)
    {
        ...
        if (!int.TryParse(numStr, out var number)) return Fail("view 需要 number 参数");

        var json = await _gh.GetIssue(owner, repo, number, ct);
        if (json.StartsWith("GitHub API")) return Fail(json);
        return Ok($"[{owner}/{repo}] Issue #{number}:\n{FormatIssueResponse(json)}");
    }

    private async Task<ToolResult> DoClose(string owner, string repo, List<string> inputs, CancellationToken ct)
    {
        var numStr = Get(inputs, 2).Trim();
        if (!int.TryParse(numStr, out var number)) return Fail("close 需要 number 参数");

        var json = await _gh.CloseIssue(owner, repo, number, ct);
        if (json.StartsWith("GitHub API")) return Fail(json);
        return Ok($"[{owner}/{repo}] Issue #{number} 已关闭");
    }
    ...
    private static string FormatIssueResponse(string json)
    {
        try
        {
            using var doc = JsonDocument.Parse(json);
