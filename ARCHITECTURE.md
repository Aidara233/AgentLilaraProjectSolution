...
字段说明：
- `tool` — 工具名称
- `toolId` — 本次调用的唯一标识，同时作为寄存器的 key
- `inputs` — 输入数组，每项为字面值（`type: "value"`）或引用其他工具输出（`type: "ref"`）
  - ref 隐含了依赖关系：引用了某个 toolId 的输出，就必须等该工具完成
  - 支持多输入
- `output` — 输出标识，存入寄存器供下游工具的 inputs 引用；无输出的工具可省略
- `outputToModel` — 是否将完整结果回传模型，默认 false。未标记的工具仅回传状态摘要
...
字段说明：
- `toolId` — 对应 ToolCall 的 toolId
- `status` — 执行状态：`success`（成功）/ `failed`（失败）/ `skipped`（因上游失败未执行）
- `data` — 工具输出数据，成功时存入寄存器供下游引用
- `error` — 失败时的错误信息

### 寄存器

工具执行的中间数据存储，用于工具间数据传递和错误恢复。

- **作用域**：绑定到单次 WorkerEngine 实例，任务结束自动清空
- **写入**：每个工具执行完毕后，以 toolId 为 key 将 ToolResult 写入寄存器
- **读取**：下游工具通过 inputs 中的 ref 读取上游寄存器数据
- **错误恢复**：DAG 部分失败时，已完成工具的数据保留在寄存器中，模型重新规划时可直接引用，无需重复执行
- **跨轮持久化**：不是寄存器的职责。需要跨轮保留的数据应由模型通过工具（如写文件）自行存储
...
4. 某个工具失败时：记录错误，继续执行其他无依赖的工具；依赖该工具的下游标记为 `skipped`
5. 所有工具完成后，汇总结果回传模型：
   - `outputToModel: true` 的工具 → 回传完整 data
   - 其余工具 → 仅回传 toolId + status，失败时附带 error

回传示例：
```
[read1] 成功
[read2] 成功，返回值：这是用户要求查看的内容...
[transform1] 失败：不支持的编码格式 GBK
[write1] 未执行（依赖 transform1）
```

模型收到结果后，可针对失败的部分重新规划 ToolCall，已成功的工具数据仍保留在寄存器中可直接引用。
...
**普通工具（无进度汇报接口）：**
- 工具注册时声明超时时间
- 到时间未完成则强制中止，按 `failed` 处理

**长时工具（实现进度汇报接口）：**
- 工具定期向 WorkerEngine 汇报进度（如 `1.0GB/2.0GB, 100MB/s`）
- WorkerEngine 监测进度变化，连续 N 次无实质进展则判定异常，中止工具，按 `failed` 处理

两种情况最终走同一条错误处理路径：中止工具 → ToolResult 标记 failed → 已完成数据保留在寄存器 → 汇总回传模型。
...
- ToolCall 重写 — 当前代码中的 ToolCall 结构（pipeIn/pipeOut/afterThan/input）需要按新设计（inputs 数组 + output + outputToModel + 寄存器 + ToolResult）重写
