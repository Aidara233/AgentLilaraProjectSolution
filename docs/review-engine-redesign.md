...
- `review_focus(message_id?, offset?, channel_id?)` — 移动游标
  - message_id：跳到指定消息（db_id，search 结果中的 `[ID:xxx]`）
  - 只传 message_id 时，若当前游标无频道，会自动按消息 ID 推导 channel_id
  - message_id + offset：跳到该消息前/后 N 条的位置（负值=往前）
  - channel_id（无 message_id）：跳到该频道最新消息
- `review_browse(count=20)` — 从游标处正序读取当前频道消息，游标前进
- browse 输出不带消息 ID（干净的阅读流：`[时间] 发送人(P#ID): 内容`）
- search 输出带 db_id（`[ID:{db_id}]`），供 focus 跳转用
...
| review_log | action, summary, detail? | 记录一次复盘审计日志 |
...
**应用时机：** review_complete 时（含主动调用和增援耗尽强制完成），每目标每维度取平均，应用一次 delta。即时写入 EvaluationScore 表。非 complete 退出（外部中断）不应用，缓冲保留至下次 resume。
...
降级自动执行（维度跌破门槛即降）。升级需要硬性条件 + 维度条件同时满足，由框架在 review_complete 时自动检查并执行。
模型不直接调用升降级工具，只通过 review_evaluate 记录评价。当人物满足硬性条件但维度未达标时，框架自动生成信标引导 Review 评估。
...
自由探索期间不注入 token 状态，模型完全沉浸于内容。

**预算耗尽 → 自动激活增援：**
```
[系统] 主预算已用尽，已自动激活增援预算 15000 tokens。请尽快收尾。
```
仅注入一次。增援无需模型主动申请。

**增援耗尽 → 强制完成：**
```
[系统] 增援预算已耗尽，评价已自动提交。复盘结束。
```
框架调用 `MarkComplete()`，下一轮 IsCompleted 判定 → 评价缓冲 Apply → 停止。
...
```json
{
  ...
  "reserveUsed": false,
  "resumeCount": 0,
  "savedAt": "2026-06-03T02:30:00"
}
```

**恢复时：** 游标回到上次位置，缓冲保留（继续累积），notes 注入，token 从 0 重新计数（新会话新预算）。`resumeCount` 递增，用于渐进催收（≥2 次时提醒尽快完成）。

**保存时机：** 仅在缓冲非空时写文件。缓冲为空（如 complete 后已清空）则删除进度文件。这意味着仅"外部中断"会留下 resumeable 的进度。

**Resume 种子内容：** 恢复时注入旧笔记 + 缓冲条目清单 + 游标位置 + resumeCount 提示。不注入新信标/候选人种子。
