...
- **SleepTalkCore** — 梦话生成：基于当前片段+触发词生成梦幻呓语(≤50字)
- **SummarizationCore** — 上下文压缩摘要：将旧消息压缩为简洁摘要
...
- 通信桥梁：TaskBridge
- 状态：CurrentSleepState(get/set) / MuteMode
...
- **睡眠拦截**（CurrentSleepState != None 时）：
  - 走神：被 @ → 放行
  - 小睡：被 @ + 叫醒关键词 → 放行
  - 大睡：管理员 @ → 发 force-wake 信号 + 放行
  - 其余：消息已入库，不触发响应（醒来后自动补提取记忆）
...
- 小睡：被 @ + 叫醒关键词("起床/醒醒/wake/起来/叫醒/别睡了/醒来") → shouldWake=true
         仅被 @ → 触发梦话（SleepTalkCore），不打断
- 大睡：消息不打断，仅 ForceWake() 可唤醒（管理员/任务提交触发）

**梦话**（SleepTalkCore）：
- 片段完成后概率触发：大睡 25% / 小睡 15% / 走神不触发
- 小睡被 @ 时必定触发一条梦话
- 内容基于当前片段 + 触发词生成梦幻呓语（≤50字）
- 尊重 MuteMode

**睡眠状态广播**：
- 启动时设置 ctx.CurrentSleepState（Daydream/Nap/DeepSleep）
- 结束时重置为 SleepState.None
- ChannelEngineSpawnCheck 据此拦截消息
...
- force-wake → 转发给活跃 DreamEngine.ForceWake()（管理员/任务唤醒）
